...
function CheckInSheet({ open, onClose, onApply }) {
  ...
  const newTargets = { ...state.targets, energy: state.targets.energy + 35, carb: state.targets.carb + 9 };
  return (
    <Sheet open={open} onClose={onClose} title="Weekly Check-In" headerRight={<Icon name="circle-check" size={20} />}>
      {stage === 0 ? (
        <div className="mf-checkin-body">
          ...
          <p className="mf-onb-lede" style={{ textAlign: 'center' }}>
            Basierend auf deinem Gewicht ({latestWeight(state)} kg) und deiner Adhärenz passen wir dein Energieziel an.
          </p>
